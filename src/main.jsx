...
function App() {
  ...
  return <main className="admin-page">
    <header className="admin-header"><div className="admin-brand"><img src={logoUrl} alt="Logo Yayasan" /><div><h1>Dashboard CMS</h1><p>Yayasan Bina Tandang Nurul Falah</p></div></div><button onClick={() => supabase.auth.signOut()}>Keluar</button></header>
    {message && <div className="notice">{message}</div>}
    {uploading && <div className="notice">Mengunggah gambar…</div>}
    <nav className="tabs" aria-label="Navigasi dashboard"><button aria-current={tab === "articles" ? "page" : undefined} className={tab === "articles" ? "active" : ""} onClick={() => setTab("articles")}>Artikel</button><button aria-current={tab === "announcements" ? "page" : undefined} className={tab === "announcements" ? "active" : ""} onClick={() => setTab("announcements")}>Pengumuman</button><button aria-current={tab === "profile" ? "page" : undefined} className={tab === "profile" ? "active" : ""} onClick={() => setTab("profile")}>Profil Yayasan</button><button type="button" className="secondary public-view" onClick={() => { window.location.href = publicUrl; }}>Tampilan Publik ↗</button></nav>
