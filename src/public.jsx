...
function PublicSite() {
  ...
  return <div style={styles.page}>
    <header style={styles.header}><div><button onClick={home} style={styles.brand}>{profile?.name || "Yayasan Bina Tandang Nurul Falah"}</button><div style={styles.tagline}>{profile?.tagline || "Berkarya, mengabdi, dan memberi manfaat"}</div></div><nav><a href="#profil" style={styles.nav}>Profil</a><a href="#artikel" style={styles.nav}>Artikel</a><a href="#pengumuman" style={styles.nav}>Pengumuman</a><a href="./admin.html" style={styles.admin}>Admin</a></nav></header>
    ...
    <footer style={styles.footer}><div><b>{profile?.name || "Yayasan Bina Tandang Nurul Falah"}</b><p>{profile?.short_description || ""}</p></div><div>{profile?.phone && <p>☎ {profile.phone}</p>}{profile?.email && <p>✉ {profile.email}</p>}</div></footer>
  </div>;
}

const styles = {
  page:{minHeight:"100vh",background:"#f7f5ef",color:"#18322a",fontFamily:"system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"},header:{position:"sticky",top:0,zIndex:5,display:"flex",justifyContent:"space-between",alignItems:"center",gap:24,padding:"18px 6vw",background:"rgba(255,255,255,.94)",backdropFilter:"blur(12px)",borderBottom:"1px solid #e6e2d8"},brand:{border:0,background:"none",padding:0,fontWeight:800,fontSize:"1.05rem",color:"#18322a",cursor:"pointer",textAlign:"left"},tagline:{fontSize:".8rem",color:"#68766f",marginTop:4},nav:{marginLeft:18,color:"#315449",textDecoration:"none",fontSize:".9rem"},admin:{marginLeft:18,textDecoration:"none",padding:"9px 14px",borderRadius:999,background:"#18322a",color:"white",fontSize:".85rem"},ticker:{padding:"10px 6vw",background:"#e8efe9",display:"flex",gap:10,alignItems:"center",fontSize:".9rem"},hero:{background:"linear-gradient(135deg,#18322a,#315c4d)",color:"white"},heroInner:{maxWidth:1100,margin:"auto",padding:"90px 6vw",display:"grid",gridTemplateColumns:"1fr 220px",gap:40,alignItems:"center"},kicker:{fontSize:".75rem",letterSpacing:".16em",fontWeight:800,opacity:.7},heroTitle:{fontSize:"clamp(2.2rem,5vw,4.6rem)",lineHeight:1.02,margin:"12px 0 18px",maxWidth:800},heroText:{fontSize:"1.1rem",lineHeight:1.7,maxWidth:700,opacity:.9},cta:{display:"inline-block",marginTop:16,padding:"12px 18px",borderRadius:999,background:"white",color:"#18322a",textDecoration:"none",fontWeight:700},heroMark:{border:"1px solid rgba(255,255,255,.35)",borderRadius:28,aspectRatio:"1",display:"grid",placeItems:"center",fontSize:"2rem",fontWeight:900,letterSpacing:".08em"},main:{maxWidth:1100,margin:"auto",padding:"56px 6vw"},section:{padding:"50px 0",scrollMarginTop:90},heading:{fontSize:"clamp(1.8rem,3vw,2.7rem)",margin:"8px 0 18px"},grid2:{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:24,margin:"28px 0"},cards:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:22},card:{background:"white",borderRadius:20,overflow:"hidden",border:"1px solid #e5e1d8",boxShadow:"0 8px 30px rgba(24,50,42,.06)"},thumb:{width:"100%",height:190,objectFit:"cover"},cardBody:{padding:22},date:{fontSize:".75rem",color:"#7b867f",textTransform:"uppercase",letterSpacing:".08em"},read:{border:0,background:"none",padding:0,color:"#315c4d",fontWeight:800,cursor:"pointer"},announcements:{display:"grid",gap:14},announcement:{background:"white",border:"1px solid #e5e1d8",borderRadius:16,padding:18,display:"flex",gap:18},banner:{width:180,maxHeight:120,objectFit:"cover",borderRadius:12},footer:{padding:"45px 6vw",background:"#18322a",color:"white",display:"flex",justifyContent:"space-between",gap:30},articleTitle:{fontSize:"clamp(2rem,5vw,4rem)",lineHeight:1.05,margin:"10px 0"},subtitle:{fontSize:"1.15rem",color:"#68766f"},cover:{width:"100%",maxHeight:520,objectFit:"cover",borderRadius:24,margin:"25px 0"},content:{fontSize:"1.08rem",lineHeight:1.9,maxWidth:780},references:{marginTop:35,padding:20,background:"#eeece5",borderRadius:16},back:{border:0,background:"none",cursor:"pointer",color:"#315c4d",fontWeight:800,padding:0},center:{minHeight:"100vh",display:"grid",placeItems:"center",padding:30,textAlign:"center"}
};
